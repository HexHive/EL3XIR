...
#dont let it run every time only once is enough
...
#cp -f "build/qemu-system-arm" "../../afl-qemu-trace" || exit 1 
cp -f "build/qemu-system-aarch64" "../../afl-qemu-system-trace" || exit 1 

echo "for now only exit..."
