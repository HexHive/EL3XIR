...
extern target_ulong       afl_entry_point, afl_start_code, afl_end_code;
...
//target_ulong afl_get_brk(void);
//target_ulong afl_set_brk(abi_ulong new_brk);
...
static inline int afl_must_instrument(target_ulong addr) {
  ...
  for (struct vmrange* n = afl_instr_code; n; n = n->next) {
    if (!n->exclude && addr < n->end && addr >= n->start){
      printf("will instrument addr:"TARGET_FMT_lx" in vmrange addr:"TARGET_FMT_lx", end:" TARGET_FMT_lx"\n",addr,n->start,n->end);
      return 1;
    }
  }
