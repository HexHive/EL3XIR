...
/*static uint64_t calc_cov_fingerprint(afl_forkserver_t *fsrv)  {
...
u32 execute_probing() {
  ACTF("AFL in probing mode!");

  uint32_t start = atoi(getenv("AFL_FUNCID_START"));
  uint32_t end = atoi(getenv("AFL_FUNCID_END"));
  //uint64_t cov_fingerprints[end-start];
  //uint64_t cov_i = 0;

  while (start < end) {
    ACTF("Running %lx", start);
    showmap_run_target_forkserver(fsrv, &start, sizeof(start));

    //uint64_t fp = calc_cov_fingerprint(fsrv);
    //ACTF("Fingerpint: %lx", fp);
    snprintf(outfile, sizeof(outfile), "%s/%s-%lx", out_file, "probe", start);
    tcnt = write_results_to_file(fsrv, outfile);
...
int main(int argc, char **argv_orig, char **envp) {
  ...
  fsrv = &fsrv_var;
  afl_fsrv_init(fsrv);
  // thats size of shared memory map where coverage is recorded
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:A:eqCZOQUWbcrsh")) > 0) {
    switch (opt) {
      ...
      case 'Q':
        ...
        fsrv->qemu_mode = (fsrv->qemu_mode + 1);
        break;
  ...
  // here the env variable SHM_ENV_VAR is set
  ...
  if (fsrv->qemu_mode) {
    if (use_wine) {
      ...
      // here we get argv to start the qemu binary later on
      use_argv = get_qemu_argv(fsrv->qemu_mode, argv[0], &fsrv->target_path,
